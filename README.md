...
- FIFO lots are reconstructed from `OPENING_BALANCE` and `BUY` trade history while `stock_holdings` remains an aggregated materialized projection. Persisted lot-match inspection is not exposed, user-selectable tax-lot matching is not supported, and LIFO or average-cost accounting are not supported.
